...
const { postId } = router.query;
  ...
  const type: PostType = router.pathname.includes("status")
    ? "status"
    : "comment";
...
const getFileDetails = async (fileId: string) => {
    ...
    //   });
    // });
    const res = await fetch(`/api/imagekit/file-details?fileId=${fileId}`);
  ...
  <div className="p-4 border-y-[1px] border-borderGray">
    ...
    <div className={`flex gap-4 ${type === "status" && "flex-col"}`}>
      ...
      <div className="flex-1 flex flex-col gap-2">
        ...
        <MediaPage path="general/post.jpeg" alt="" w={600} h={600} />
